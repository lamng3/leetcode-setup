...
public:
    ...
    vi coordinate_compress(const vii& pairs) {
        set<int> uq;
        for (auto& p : pairs) {
            uq.insert(p[0]);
            uq.insert(p[1]);
    ...
    vii validArrangement(vii& pairs) {
        ...
        vi A = coordinate_compress(pairs);

        int m = (int)A.size();
        REP(i, m) rank[A[i]] = i;

        g.assign(m, vector<pii>());
        vi in(m, 0), out(m, 0);

        for (int i = 0; i < n; i++) {
            int u = rank[pairs[i][0]], v = rank[pairs[i][1]];
            g[u].pb({v, i});
            out[u]++;
            in[v]++;
        }
        ...
        vii ans;
        for (int i = 1; i < path.size(); i++) ans.pb({A[path[i-1]], A[path[i]]});
        return ans;
    }
};
