...
struct BSTIteratorReversed {
    ...
    BSTIteratorReversed(TreeNode* node) : root(node) {
        traverse(root);
    }
...
public:
    bool findTarget(TreeNode* root, int k) {
        BSTIterator iterLow(root);
        BSTIteratorReversed iterHigh(root);
    
        int L = iterLow.next(), R = iterHigh.next();

        while (L < R) {
            int S = L + R;
            if (S == k) return true;
            if (S < k) L = iterLow.next();
            else R = iterHigh.next();
        }
