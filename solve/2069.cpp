...
public:
    ...
    vi getPos() {
        if (rxy <= W) return {rxy, 0};
        if (rxy <= W + H) return {W, rxy - W};
        if (rxy <= 2 * W + H) return {2 * W + H - rxy, H};
        return {0, P - rxy};
    }
    
    string getDir() {
        if (rxy == 0) return moved ? "South" : "East";
        if (rxy <= W) return "East";
        if (rxy <= W + H) return "North";
        if (rxy <= 2 * W + H) return "West";
        return "South";
    }
};
